...
describe('Security Testing Suite', () => {
  ...
  describe('Route Access Control', () => {
    it('should return 404 for undefined routes', () => {
      return request(getServer()).get('/api/undefined-route-xyz').expect(404);
    });

    it('should return 401 or 404 for malformed customer routes without auth', () => {
      return request(getServer()).get('/api/customers/-1').expect([401, 404]); // 401 because auth is required; 404 if somehow bypassed
    });
  });
  ...
  describe('CORS Headers', () => {
    ...
    it('should handle preflight requests gracefully', () => {
      return request(getServer()).options('/api').expect([200, 204, 404]); // 404 is acceptable if OPTIONS not explicitly handled
    });
  });
  ...
  describe('HTTP Method Security', () => {
    ...
    it('DELETE to root should not work', () => {
      return request(getServer()).delete('/api').expect([404, 400, 405]);
    });
  });
  ...
  describe('Endpoint Availability', () => {
    it('should have customers endpoint accessible', () => {
      return request(getServer()).get('/api/customers').expect([200, 401, 500]); // May need auth or have issues
    });

    it('should have users endpoint accessible', () => {
      return request(getServer()).get('/api/users').expect([200, 401, 500]);
    });

    it('should have vehicles endpoint accessible', () => {
      return request(getServer()).get('/api/vehicles').expect([200, 401, 500]);
    });
  });
  ...
  describe('Application Stability', () => {
    ...
    it('should handle concurrent requests', () => {
      return Promise.all([
        request(getServer()).get('/api').expect(200),
        request(getServer()).get('/api').expect(200),
        request(getServer()).get('/api').expect(200),
      ]);
    });
