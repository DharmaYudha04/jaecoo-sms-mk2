export function validateEnvironmentVariables(): void {
  const isServerless =
    process.env.VERCEL === '1' || process.env.AWS_REGION !== undefined;
  ...
  const invalidFrontendOrigin = frontendOrigins.find(
  ...
  if (!isServerless && !Number.isFinite(Number(process.env.PORT || ''))) {
