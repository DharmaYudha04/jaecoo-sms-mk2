...
export class SchedulesService {
  ...
  async getMechanics(role?: string) {
    if (role !== undefined && !VALID_ROLES.includes(role as Role)) {
      throw new BadRequestException(
        `Role tidak valid. Gunakan salah satu dari: ${VALID_ROLES.join(', ')}.`,
      );
    }
  ...
  async getByDate(date: string) {
    ...
    return this.prisma.servis.findMany({
      ...
      include: {
        workOrder: {
          select: { id_wo: true, no_rangka: true, nomor_wo_pusat: true },
        },
        detail_servis: true,
      },
  ...
  async getUpcoming(daysAhead: number = 7) {
    const days = Math.max(
      1,
      Math.min(Number.isFinite(daysAhead) ? daysAhead : 7, 90),
    );
    const now = new Date();
