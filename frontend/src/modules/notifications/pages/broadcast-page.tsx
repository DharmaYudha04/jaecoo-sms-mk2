...
export function BroadcastPage() {
  ...
  const status: 'all' | 'unread' = statusParam === 'unread' ? 'unread' : 'all';

  const sortParam = searchParams.get('sort');
  const sort: 'oldest' | 'newest' = sortParam === 'oldest' ? 'oldest' : 'newest';
  ...
  const openBroadcast = async (item: AppNotification) => {
    ...
    try {
      ...
      showToast({
        title: 'Gagal membuka broadcast',
        description: getErrorMessage(error, 'Broadcast tidak berhasil dibuka.'),
        tone: 'error',
      });
    } finally {
    ...
    <div className="space-y-5">
        ...
        actions={
          <>
              ...
              onClick={async () => {
                ...
                try {
                  ...
                  showToast({
                    ...
                    description: 'Semua broadcast berhasil dihapus.',
                    tone: 'success',
                  });
                  updateParams({ page: '1' });
                } catch (error) {
                  showToast({
                    title: 'Gagal menghapus broadcast',
                    description: getErrorMessage(error, 'Terjadi kesalahan saat menghapus broadcast.'),
                    tone: 'error',
                  });
                }
              }}
            ...
            <Button
              ...
              disabled={!hasBroadcasts || markAllMutation.isPending || clearBroadcastsMutation.isPending}
            >
      ...
      <Card className="grid gap-3 md:grid-cols-[1.6fr_0.8fr_0.8fr]">
        <form onSubmit={submitSearch} className="flex items-center gap-2">
          ...
          <Button
            type="submit"
            variant="secondary"
            className="action-icon-button search-icon-button shrink-0"
            aria-label="Telusuri broadcast"
            title="Telusuri broadcast"
          >
        ...
        <Select
          value={status}
          onChange={(event) => updateParams({ status: event.target.value === 'unread' ? 'unread' : null })}
        >
        ...
        <Select
          value={sort}
          onChange={(event) => updateParams({ sort: event.target.value === 'oldest' ? 'oldest' : null })}
        >
      ...
      <Card className="space-y-3">
        {broadcastsQuery.isLoading ? (
          <LoadingState message="Memuat broadcast..." rows={4} />
        ) : broadcastsQuery.isError ? (
          <EmptyState message={getErrorMessage(broadcastsQuery.error, 'Gagal memuat broadcast.')} />
        ) : !broadcasts.length ? (
          <EmptyState message="Belum ada broadcast." />
        ) : (
          <div className="space-y-4">
            <div className="space-y-3">
              {pagedBroadcasts.map((item) => (
                <div
                  ...
                  className={[
                    'rounded-[24px] p-5',
                    item.isRead ? 'notification-surface-read' : 'notification-surface-unread',
                  ].join(' ')}
                >
                  <div className="flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between">
                    <button
                      ...
                      disabled={
                        ...
                        clearBroadcastsMutation.isPending ||
                        openingId === item.id
                      }
                    >
                    ...
                    <div className="flex shrink-0 flex-wrap items-center gap-2">
                      <Button
                        ...
                        className={
                          item.isRead
                            ? 'notification-action-button notification-action-button--done'
                            : 'notification-action-button notification-action-button--read'
                        }
                        disabled={
                          item.isRead ||
                          pendingReadId === item.id ||
                        ...
                        onClick={async () => {
                          ...
                          try {
                            await markReadMutation.mutateAsync(item.id);
                            showToast({
                              title: 'Broadcast ditandai dibaca',
                              description: 'Status broadcast telah diperbarui.',
                              tone: 'success',
                            });
                          } catch (error) {
                            if (isAlreadyHandledElsewhere(error)) {
                              showToast({
                                title: 'Broadcast sudah diperbarui',
                                description: 'Item ini sudah berubah dari perangkat atau sesi lain.',
                                tone: 'info',
                              });
                            } else {
                              showToast({
                                title: 'Gagal menandai broadcast',
                                description: getErrorMessage(error, 'Broadcast tidak berhasil ditandai sebagai dibaca.'),
                                tone: 'error',
                              });
                            }
                          } finally {
                      ...
                      <Button
                        ...
                        onClick={async () => {
                          ...
                          try {
                            ...
                            showToast({
                              title: 'Broadcast dihapus',
                              description: 'Broadcast telah dihapus dari akun Anda.',
                              tone: 'success',
                            });
                            ...
                            if (isAlreadyHandledElsewhere(error)) {
                              showToast({
                                title: 'Broadcast sudah tidak tersedia',
                                description: 'Item ini sudah dihapus dari perangkat atau sesi lain.',
                                tone: 'info',
                              });
                              ...
                              showToast({
                                title: 'Gagal menghapus broadcast',
                                description: getErrorMessage(error, 'Broadcast tidak berhasil dihapus.'),
                                tone: 'error',
                              });
                            }
                          } finally {
            ...
            <div className="flex flex-wrap items-center justify-between gap-3 rounded-[20px] border border-[color:var(--line)] bg-[color:var(--panel-light)] px-4 py-3 text-sm theme-muted">
              <span>
                Halaman {currentPage} dari {totalPages} • {broadcasts.length} data
              </span>
